...
import InteractiveMapUtils from '../../InteractiveMapUtils.ts'
...
export default class Token extends Graphics {
	...
	private init(x: number, y: number): void {
		const pos = InteractiveMapUtils.coordinatesToPixelPosition(x, y)
	...
	private onDragMove(event: FederatedPointerEvent) {
		if (this._isDragging) {
			...
			// Snap Token to Cell
			this.position = InteractiveMapUtils.snapToCell(
				this.position.x,
				this.position.y
			)
		}
